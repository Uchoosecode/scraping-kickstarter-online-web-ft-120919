...
def create_project_hash
  ...
  kickstarter.css("li.project.grid_4").each do |project| title = projects.css("h2.bbcard_name strong a").text
    
    projects[title.to_sym] = { :image_link = projects.css("div.project-thumbnail a img").attribute("src").value,
    
    :description = projects.css("p.bbcard_blurb").text,
    
    :location = projects.css("ul.project-meta span.location-name").text,
